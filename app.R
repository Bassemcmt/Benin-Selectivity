...
ui <- 
  secure_app(head_auth = tags$script(inactivity),
             ...
             dashboardPage(
               dashboardHeader(
                 title = "Bénin Select",
                 # Autres éléments de la barre supérieure
                 # ...
                 
                 # Ajouter votre nom
                 tags$li(class = "dropdown",
                         tags$a(href = "#", class = "dropdown-toggle", "Bassem CHERMITI WF", 
               ...
               dashboardSidebar(
                 sidebarMenu(
                   ...
                   menuItem("Télechargement", tabName = "files",
                            icon = icon("download"),
                            fileInput("file1", "Fichier déclarations XLSX", multiple = TRUE, accept = ".xlsx"),
                            fileInput("file2", "Fichier AVD XLSX", multiple = TRUE, accept = ".xlsx"),
                            fileInput("file3", "Fichier droits Compromis XLSX", multiple = TRUE, accept = ".xlsx")
                   ),
...
server <- function(input, output, session) {
  tryCatch({
    ...
    Droits_compromis <- reactive({
      ...
      colnames(Droits_compromis) <- c("BUREAU_D",	"NUM_DECL",	"DATE_ENR",	"NUM_ARTICLE",	"DC")
    ...
    df <- reactive({ 
      ...
      return(df)
        ...
        showModal(modalDialog(
          ...
          " Veuillez importés correctement tous les 3 fichiers nécessaires en cliquant sur Télecharger"
        ))
        # Retourner un dataframe vide ou NULL
        NULL
      })
    })
    ...
    output$Oper_Evaluation <- renderDataTable({
      ...
      nbre_dec_oper_nc <- nbre_dec_oper_nc %>%
       ...
       lst_operateur <- data %>%
        ...
        group_by(CODE_IMPORTATEUR,NOM_IMPORTATEUR) %>% distinct(CODE_IMPORTATEUR)
    ...
    shinyalert("Verifie données", type = "error")
  }
  ) 
